object telaCadPos: TtelaCadPos
  Left = 619
  Top = 226
  Width = 525
  Height = 369
  Caption = 'Cadastro de POS'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 509
    Height = 331
    Align = alClient
    TabOrder = 0
    object Label2: TLabel
      Left = 8
      Top = 16
      Width = 62
      Height = 13
      Caption = 'DESCRICAO'
      FocusControl = editDesc
    end
    object Label3: TLabel
      Left = 8
      Top = 56
      Width = 38
      Height = 13
      Caption = 'SERIAL'
      FocusControl = editSerial
    end
    object btnSair: TSpeedButton
      Left = 356
      Top = 8
      Width = 100
      Height = 37
      Action = actSair
      Glyph.Data = {
        
        F8F8FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
    end
    object btnInserir: TSpeedButton
      Left = 3
      Top = 288
      Width = 105
      Height = 37
      Action = actInserir
      Glyph.Data = {
        
        9E9EFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
    end
    object btnEdit: TSpeedButton
      Left = 140
      Top = 288
      Width = 100
      Height = 37
      Action = actEditar
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
    end
    object btnExcluir: TSpeedButton
      Left = 271
      Top = 288
      Width = 100
      Height = 37
      Action = actExcluir
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
    end
    object btnCadastrar: TSpeedButton
      Left = 400
      Top = 288
      Width = 100
      Height = 37
      Action = actCadastrar
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
    end
    object Label1: TLabel
      Left = 139
      Top = 56
      Width = 105
      Height = 13
      Caption = 'CHAVE REQUISI'#199#195'O'
      FocusControl = edtChReq
    end
    object btnGeraChave: TSpeedButton
      Left = 463
      Top = 66
      Width = 29
      Height = 29
      Enabled = False
      Glyph.Data = {
        
        A0A0A0A0A0A0A0A0E7E7E7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      OnClick = btnGeraChaveClick
    end
    object DBGrid1: TDBGrid
      Left = 4
      Top = 104
      Width = 493
      Height = 176
      DataSource = telaDados.dsPos
      Enabled = False
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
    end
    object editDesc: TDBEdit
      Left = 8
      Top = 32
      Width = 264
      Height = 21
      CharCase = ecUpperCase
      DataField = 'DESCRICAO'
      DataSource = telaDados.dsPos
      Enabled = False
      TabOrder = 1
    end
    object editSerial: TDBEdit
      Left = 8
      Top = 72
      Width = 121
      Height = 21
      CharCase = ecUpperCase
      DataField = 'SERIAL'
      DataSource = telaDados.dsPos
      Enabled = False
      TabOrder = 2
    end
    object edtChReq: TDBEdit
      Left = 138
      Top = 70
      Width = 315
      Height = 21
      DataField = 'CHREQ'
      DataSource = telaDados.dsPos
      Enabled = False
      TabOrder = 3
    end
  end
  object ActionList1: TActionList
    Left = 280
    Top = 8
    object actInserir: TAction
      Caption = 'F1 - &Incluir'
      ShortCut = 112
      OnExecute = actInserirExecute
    end
    object actEditar: TAction
      Caption = 'F2 - E&ditar'
      ShortCut = 113
      OnExecute = actEditarExecute
    end
    object actExcluir: TAction
      Caption = 'F3 - E&xcluir'
      ShortCut = 114
      OnExecute = actExcluirExecute
    end
    object actCadastrar: TAction
      Caption = 'F7 - &Salvar'
      ShortCut = 118
      OnExecute = actCadastrarExecute
    end
    object actSair: TAction
      Caption = 'F7 - &Fechar'
      ShortCut = 118
      OnExecute = actSairExecute
    end
  end
end
